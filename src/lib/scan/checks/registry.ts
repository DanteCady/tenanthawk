import type { Category } from "@/db/types";

export type ScanSector =
  | "identity"
  | "cost"
  | "teams"
  | "sharepoint"
  | "exchange"
  | "devices"
  | "apps"
  | "copilot";

export type CheckTier = "v1" | "v2" | "informational" | "backlog";
export type ScoreImpact = "full" | "informational" | "none";

export interface CheckDefinition {
  id: string;
  sector: ScanSector;
  tier: CheckTier;
  category: Category;
  module: string;
  permissions: string[];
  scoreImpact: ScoreImpact;
  marketingLabel: string;
  offered: boolean;
  exclusivityGroup?: string;
  /** Lower order wins when severities tie within the same exclusivity group. */
  exclusivityOrder?: number;
  graphValidationRef?: string;
}

/** Registry of all scan checks — single source of truth for metadata. */
export const CHECK_DEFINITIONS: CheckDefinition[] = [
  // --- Existing (15) ---
  {
    id: "security.conditional-access",
    sector: "identity",
    tier: "v1",
    category: "security",
    module: "security.ts",
    permissions: ["Policy.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Conditional Access coverage",
    offered: true,
  },
  {
    id: "security.legacy-auth",
    sector: "identity",
    tier: "v1",
    category: "security",
    module: "security.ts",
    permissions: ["Policy.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Legacy authentication",
    offered: true,
  },
  {
    id: "security.mfa-registration",
    sector: "identity",
    tier: "v1",
    category: "security",
    module: "identity.ts",
    permissions: ["Reports.Read.All"],
    scoreImpact: "full",
    marketingLabel: "MFA registration gaps",
    offered: true,
  },
  {
    id: "security.admin-roles",
    sector: "identity",
    tier: "v1",
    category: "security",
    module: "security.ts",
    permissions: ["Directory.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Privileged role assignments",
    offered: true,
  },
  {
    id: "security.guest-accounts",
    sector: "identity",
    tier: "v1",
    category: "security",
    module: "security.ts",
    permissions: ["User.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Guest account sprawl",
    offered: true,
    exclusivityGroup: "guests",
  },
  {
    id: "cost.disabled-user-licenses",
    sector: "cost",
    tier: "v1",
    category: "cost",
    module: "cost.ts",
    permissions: ["User.Read.All", "Organization.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Licenses on disabled accounts",
    offered: true,
  },
  {
    id: "cost.unused-licenses",
    sector: "cost",
    tier: "v1",
    category: "cost",
    module: "cost.ts",
    permissions: ["Organization.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Unused license seats",
    offered: true,
  },
  {
    id: "cost.license-expiry",
    sector: "cost",
    tier: "v1",
    category: "cost",
    module: "cost.ts",
    permissions: ["Organization.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Subscription status warnings",
    offered: true,
  },
  {
    id: "reliability.expiring-secrets",
    sector: "apps",
    tier: "v1",
    category: "reliability",
    module: "applications.ts",
    permissions: ["Application.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Expiring app secrets and certs",
    offered: true,
  },
  {
    id: "reliability.intune-stale-sync",
    sector: "devices",
    tier: "v1",
    category: "reliability",
    module: "intune.ts",
    permissions: ["DeviceManagementManagedDevices.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Stale Intune device sync",
    offered: true,
  },
  {
    id: "hygiene.inactive-users",
    sector: "identity",
    tier: "v1",
    category: "hygiene",
    module: "identity.ts",
    permissions: ["User.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Inactive user accounts",
    offered: true,
  },
  {
    id: "hygiene.disabled-outside-group",
    sector: "identity",
    tier: "v1",
    category: "hygiene",
    module: "identity.ts",
    permissions: ["Directory.Read.All", "User.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Disabled users outside offboarding groups",
    offered: true,
  },
  {
    id: "hygiene.empty-groups",
    sector: "teams",
    tier: "v1",
    category: "hygiene",
    module: "hygiene.ts",
    permissions: ["Directory.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Empty groups",
    offered: true,
  },
  {
    id: "hygiene.intune-noncompliant",
    sector: "devices",
    tier: "v1",
    category: "hygiene",
    module: "intune.ts",
    permissions: ["DeviceManagementManagedDevices.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Non-compliant Intune devices",
    offered: true,
  },
  {
    id: "hygiene.entra-unmanaged-devices",
    sector: "devices",
    tier: "v1",
    category: "hygiene",
    module: "devices.ts",
    permissions: ["Device.Read.All", "DeviceManagementManagedDevices.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Entra devices not in Intune",
    offered: true,
    graphValidationRef: "DV1",
  },
  {
    id: "hygiene.intune-ghost-enrollment",
    sector: "devices",
    tier: "v1",
    category: "hygiene",
    module: "devices.ts",
    permissions: ["Device.Read.All", "DeviceManagementManagedDevices.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Intune enrollments without Entra device",
    offered: true,
    graphValidationRef: "DV1",
  },
  {
    id: "reliability.entra-stale-devices",
    sector: "devices",
    tier: "v1",
    category: "reliability",
    module: "devices.ts",
    permissions: ["Device.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Stale Entra device sign-ins",
    offered: true,
    graphValidationRef: "DV1",
  },
  {
    id: "hygiene.personal-device-enrolled",
    sector: "devices",
    tier: "v1",
    category: "hygiene",
    module: "devices.ts",
    permissions: ["DeviceManagementManagedDevices.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Personally owned Intune enrollments",
    offered: true,
  },
  {
    id: "hygiene.sharing",
    sector: "sharepoint",
    tier: "v1",
    category: "hygiene",
    module: "sharepoint.ts",
    permissions: ["SharePointTenantSettings.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Org-wide SharePoint sharing policy",
    offered: true,
  },
  {
    id: "hygiene.sharepoint-external-sites",
    sector: "sharepoint",
    tier: "v1",
    category: "hygiene",
    module: "sharepoint.ts",
    permissions: ["Reports.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Sites with external sharing",
    offered: true,
    graphValidationRef: "SP1",
  },
  {
    id: "hygiene.sharepoint-stale-sites",
    sector: "sharepoint",
    tier: "v1",
    category: "hygiene",
    module: "sharepoint.ts",
    permissions: ["Reports.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Stale SharePoint sites",
    offered: true,
    graphValidationRef: "SP1",
  },
  {
    id: "hygiene.sharepoint-high-storage",
    sector: "sharepoint",
    tier: "v1",
    category: "hygiene",
    module: "sharepoint.ts",
    permissions: ["Reports.Read.All"],
    scoreImpact: "full",
    marketingLabel: "High-storage SharePoint sites",
    offered: true,
    graphValidationRef: "SP1",
  },
  {
    id: "hygiene.sharepoint-empty-sites",
    sector: "sharepoint",
    tier: "v1",
    category: "hygiene",
    module: "sharepoint.ts",
    permissions: ["Reports.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Empty SharePoint sites",
    offered: true,
    graphValidationRef: "SP1",
  },
  {
    id: "hygiene.sharepoint-ownerless-sites",
    sector: "sharepoint",
    tier: "v1",
    category: "hygiene",
    module: "sharepoint.ts",
    permissions: ["Reports.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Ownerless SharePoint sites",
    offered: true,
    graphValidationRef: "SP1",
  },
  {
    id: "hygiene.sharepoint-inactive-files",
    sector: "sharepoint",
    tier: "v1",
    category: "hygiene",
    module: "sharepoint.ts",
    permissions: ["Reports.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Sites with inactive files",
    offered: true,
    graphValidationRef: "SP1",
  },
  // --- New (expansion) ---
  {
    id: "hygiene.ownerless-groups",
    sector: "teams",
    tier: "v1",
    category: "hygiene",
    module: "collaboration.ts",
    permissions: ["Directory.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Ownerless groups",
    offered: true,
  },
  {
    id: "hygiene.ownerless-teams",
    sector: "teams",
    tier: "v1",
    category: "hygiene",
    module: "collaboration.ts",
    permissions: ["Directory.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Ownerless Microsoft Teams",
    offered: true,
  },
  {
    id: "hygiene.empty-teams",
    sector: "teams",
    tier: "v1",
    category: "hygiene",
    module: "collaboration.ts",
    permissions: ["Directory.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Empty Microsoft Teams",
    offered: true,
  },
  {
    id: "hygiene.stale-teams",
    sector: "teams",
    tier: "v1",
    category: "hygiene",
    module: "collaboration.ts",
    permissions: ["Reports.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Stale Microsoft Teams",
    offered: true,
    exclusivityGroup: "teams.stale",
    graphValidationRef: "TM1",
  },
  {
    id: "hygiene.teams-no-active-channels",
    sector: "teams",
    tier: "v1",
    category: "hygiene",
    module: "collaboration.ts",
    permissions: ["Reports.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Teams with no active channels",
    offered: true,
    exclusivityGroup: "teams.activity",
    graphValidationRef: "TM1",
  },
  {
    id: "hygiene.teams-guest-heavy",
    sector: "teams",
    tier: "v1",
    category: "hygiene",
    module: "collaboration.ts",
    permissions: ["Reports.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Guest-heavy Teams",
    offered: true,
    graphValidationRef: "TM1",
  },
  {
    id: "hygiene.groups-naming-chaos",
    sector: "teams",
    tier: "informational",
    category: "hygiene",
    module: "collaboration.ts",
    permissions: ["Directory.Read.All"],
    scoreImpact: "informational",
    marketingLabel: "Groups with risky naming patterns",
    offered: true,
  },
  {
    id: "cost.unused-copilot-licenses",
    sector: "copilot",
    tier: "v1",
    category: "cost",
    module: "copilot.ts",
    permissions: ["Organization.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Unused Copilot license seats",
    offered: true,
    graphValidationRef: "CP2",
  },
  {
    id: "cost.copilot-licensed-inactive",
    sector: "copilot",
    tier: "v1",
    category: "cost",
    module: "copilot.ts",
    permissions: ["Reports.Read.All", "User.Read.All", "Organization.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Licensed inactive Copilot users",
    offered: true,
    exclusivityGroup: "copilot.adoption",
    exclusivityOrder: 1,
    graphValidationRef: "CP1",
  },
  {
    id: "cost.copilot-assigned-disabled-user",
    sector: "copilot",
    tier: "v1",
    category: "cost",
    module: "copilot.ts",
    permissions: ["User.Read.All", "Organization.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Copilot on disabled accounts",
    offered: true,
    graphValidationRef: "CP2",
  },
  {
    id: "hygiene.copilot-low-adoption",
    sector: "copilot",
    tier: "v1",
    category: "hygiene",
    module: "copilot.ts",
    permissions: ["Reports.Read.All", "User.Read.All", "Organization.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Low Copilot adoption",
    offered: true,
    exclusivityGroup: "copilot.adoption",
    exclusivityOrder: 2,
    graphValidationRef: "CP1",
  },
  {
    id: "hygiene.copilot-app-skew",
    sector: "copilot",
    tier: "v1",
    category: "hygiene",
    module: "copilot.ts",
    permissions: ["Reports.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Copilot Chat-only users",
    offered: true,
    graphValidationRef: "CP1",
  },
  {
    id: "security.copilot-licensed-no-mfa",
    sector: "copilot",
    tier: "v1",
    category: "security",
    module: "copilot.ts",
    permissions: ["User.Read.All", "Organization.Read.All", "Reports.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Copilot users without MFA",
    offered: true,
    graphValidationRef: "CP2",
  },
  {
    id: "copilot.readiness-blockers",
    sector: "copilot",
    tier: "v1",
    category: "hygiene",
    module: "copilot.ts",
    permissions: [],
    scoreImpact: "full",
    marketingLabel: "Copilot readiness blockers",
    offered: true,
  },
  {
    id: "hygiene.copilot-chat-only-usage",
    sector: "copilot",
    tier: "informational",
    category: "hygiene",
    module: "copilot.ts",
    permissions: ["Reports.Read.All"],
    scoreImpact: "informational",
    marketingLabel: "Copilot Chat-only usage pattern",
    offered: true,
    graphValidationRef: "CP1",
  },
  {
    id: "hygiene.mailbox-forwarding-external",
    sector: "exchange",
    tier: "v1",
    category: "hygiene",
    module: "exchange.ts",
    permissions: ["Mail.ReadBasic.All", "User.Read.All"],
    scoreImpact: "full",
    marketingLabel: "External mailbox forwarding",
    offered: true,
    exclusivityGroup: "forwarding",
    graphValidationRef: "EX1",
  },
  {
    id: "hygiene.mailbox-forwarding-enabled",
    sector: "exchange",
    tier: "v1",
    category: "hygiene",
    module: "exchange.ts",
    permissions: ["Mail.ReadBasic.All", "User.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Mailbox forwarding enabled",
    offered: true,
    exclusivityGroup: "forwarding",
    graphValidationRef: "EX1",
  },
  {
    id: "hygiene.inactive-mailboxes",
    sector: "exchange",
    tier: "v1",
    category: "hygiene",
    module: "exchange.ts",
    permissions: ["Reports.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Inactive mailboxes",
    offered: true,
    exclusivityGroup: "mailbox.inactive",
    graphValidationRef: "EX2",
  },
  {
    id: "hygiene.mailbox-high-storage",
    sector: "exchange",
    tier: "v1",
    category: "hygiene",
    module: "exchange.ts",
    permissions: ["Reports.Read.All"],
    scoreImpact: "full",
    marketingLabel: "High-storage mailboxes",
    offered: true,
    graphValidationRef: "EX2",
  },
  {
    id: "cost.inactive-mailbox-licenses",
    sector: "exchange",
    tier: "v1",
    category: "cost",
    module: "exchange.ts",
    permissions: ["Reports.Read.All", "User.Read.All", "Organization.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Licensed inactive mailboxes",
    offered: true,
    graphValidationRef: "EX2",
  },
  {
    id: "reliability.service-principal-secrets",
    sector: "apps",
    tier: "v1",
    category: "reliability",
    module: "applications.ts",
    permissions: ["Application.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Expiring enterprise app secrets",
    offered: true,
    graphValidationRef: "AP1",
  },
  {
    id: "security.over-permissioned-apps",
    sector: "apps",
    tier: "v1",
    category: "security",
    module: "applications.ts",
    permissions: ["Application.Read.All", "Directory.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Over-permissioned enterprise apps",
    offered: true,
    graphValidationRef: "AP2",
  },
  {
    id: "hygiene.unused-enterprise-apps",
    sector: "apps",
    tier: "v1",
    category: "hygiene",
    module: "applications.ts",
    permissions: ["Application.Read.All", "AuditLog.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Unused enterprise apps",
    offered: true,
    graphValidationRef: "AP3",
  },
  {
    id: "hygiene.app-without-owners",
    sector: "apps",
    tier: "v1",
    category: "hygiene",
    module: "applications.ts",
    permissions: ["Application.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Ownerless app registrations",
    offered: true,
    graphValidationRef: "AP1",
  },
  {
    id: "hygiene.enterprise-app-no-owners",
    sector: "apps",
    tier: "v1",
    category: "hygiene",
    module: "applications.ts",
    permissions: ["Application.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Ownerless enterprise apps",
    offered: true,
    graphValidationRef: "AP1",
  },
  {
    id: "security.app-global-admin-role",
    sector: "apps",
    tier: "v1",
    category: "security",
    module: "applications.ts",
    permissions: ["Directory.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Apps with Global Administrator role",
    offered: true,
    graphValidationRef: "AP4",
  },
  {
    id: "hygiene.multi-tenant-apps",
    sector: "apps",
    tier: "v1",
    category: "hygiene",
    module: "applications.ts",
    permissions: ["Application.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Multi-tenant apps with credentials",
    offered: true,
    graphValidationRef: "AP1",
  },
  {
    id: "security.stale-guests",
    sector: "identity",
    tier: "v1",
    category: "security",
    module: "identity-extended.ts",
    permissions: ["User.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Stale guest accounts",
    offered: true,
    exclusivityGroup: "guests",
    graphValidationRef: "ID2",
  },
  {
    id: "security.guest-invite-sprawl",
    sector: "identity",
    tier: "v1",
    category: "security",
    module: "identity-extended.ts",
    permissions: ["User.Read.All"],
    scoreImpact: "informational",
    marketingLabel: "Guest invitation spike",
    offered: true,
    graphValidationRef: "ID2",
  },
  {
    id: "hygiene.users-no-manager",
    sector: "identity",
    tier: "v1",
    category: "hygiene",
    module: "identity-extended.ts",
    permissions: ["User.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Users without a manager",
    offered: true,
    graphValidationRef: "ID2",
  },
  {
    id: "security.privileged-user-no-mfa",
    sector: "identity",
    tier: "v1",
    category: "security",
    module: "identity-extended.ts",
    permissions: ["Directory.Read.All", "Reports.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Privileged users without MFA",
    offered: true,
    graphValidationRef: "ID3",
  },
  {
    id: "security.pim-standing-access",
    sector: "identity",
    tier: "v1",
    category: "security",
    module: "identity-extended.ts",
    permissions: ["RoleManagement.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Standing privileged role access",
    offered: true,
    graphValidationRef: "ID1",
  },
  {
    id: "hygiene.inactive-channels",
    sector: "teams",
    tier: "v2",
    category: "hygiene",
    module: "teams-deep.ts",
    permissions: ["Team.ReadBasic.All", "ChannelMessage.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Inactive Teams channels",
    offered: true,
    graphValidationRef: "TM2",
  },
  {
    id: "hygiene.empty-channels",
    sector: "teams",
    tier: "v2",
    category: "hygiene",
    module: "teams-deep.ts",
    permissions: ["Team.ReadBasic.All", "ChannelMessage.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Empty Teams channels",
    offered: true,
    graphValidationRef: "TM2",
  },
  {
    id: "hygiene.private-channels-ownerless",
    sector: "teams",
    tier: "v2",
    category: "hygiene",
    module: "teams-deep.ts",
    permissions: ["Team.ReadBasic.All", "Channel.ReadBasic.All"],
    scoreImpact: "full",
    marketingLabel: "Ownerless private channels",
    offered: true,
    graphValidationRef: "TM2",
  },
  {
    id: "hygiene.teams-disabled-owner",
    sector: "teams",
    tier: "v2",
    category: "hygiene",
    module: "teams-deep.ts",
    permissions: ["User.Read.All", "Team.ReadBasic.All"],
    scoreImpact: "full",
    marketingLabel: "Teams with disabled owners",
    offered: true,
  },
  {
    id: "hygiene.teams-outdated-apps",
    sector: "teams",
    tier: "v2",
    category: "hygiene",
    module: "teams-deep.ts",
    permissions: ["Team.ReadBasic.All"],
    scoreImpact: "full",
    marketingLabel: "Outdated Teams apps",
    offered: true,
    graphValidationRef: "TM3",
  },
  {
    id: "hygiene.teams-unverified-apps",
    sector: "teams",
    tier: "v2",
    category: "hygiene",
    module: "teams-deep.ts",
    permissions: ["Team.ReadBasic.All"],
    scoreImpact: "full",
    marketingLabel: "Unverified Teams apps",
    offered: true,
    graphValidationRef: "TM3",
  },
  {
    id: "hygiene.sharepoint-unused-pages",
    sector: "sharepoint",
    tier: "v2",
    category: "hygiene",
    module: "sharepoint-deep.ts",
    permissions: ["Reports.Read.All"],
    scoreImpact: "full",
    marketingLabel: "SharePoint sites with unused pages",
    offered: true,
    graphValidationRef: "SP1",
  },
  {
    id: "hygiene.sharepoint-stale-site-pages",
    sector: "sharepoint",
    tier: "v2",
    category: "hygiene",
    module: "sharepoint-deep.ts",
    permissions: ["Reports.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Stale SharePoint site pages",
    offered: true,
    graphValidationRef: "SP1",
  },
  {
    id: "hygiene.sharepoint-anonymous-links",
    sector: "sharepoint",
    tier: "v2",
    category: "hygiene",
    module: "sharepoint-deep.ts",
    permissions: ["Reports.Read.All"],
    scoreImpact: "full",
    marketingLabel: "SharePoint anonymous links",
    offered: true,
    graphValidationRef: "SP1",
  },
  {
    id: "hygiene.onedrive-stale",
    sector: "sharepoint",
    tier: "v2",
    category: "hygiene",
    module: "sharepoint-deep.ts",
    permissions: ["Reports.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Stale OneDrive accounts",
    offered: true,
    graphValidationRef: "SP1",
  },
  {
    id: "cost.onedrive-high-storage",
    sector: "sharepoint",
    tier: "v2",
    category: "cost",
    module: "sharepoint-deep.ts",
    permissions: ["Reports.Read.All"],
    scoreImpact: "full",
    marketingLabel: "High-storage OneDrive accounts",
    offered: true,
    graphValidationRef: "SP1",
  },
  {
    id: "cost.stale-team-sites-storage",
    sector: "sharepoint",
    tier: "v2",
    category: "cost",
    module: "sharepoint-deep.ts",
    permissions: ["Reports.Read.All"],
    scoreImpact: "full",
    marketingLabel: "High-storage stale team sites",
    offered: true,
    graphValidationRef: "SP1",
  },
  {
    id: "hygiene.shared-mailbox-no-delegate",
    sector: "exchange",
    tier: "v2",
    category: "hygiene",
    module: "exchange-deep.ts",
    permissions: ["Reports.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Shared mailboxes without delegates",
    offered: true,
    graphValidationRef: "EX2",
  },
  {
    id: "hygiene.shared-mailbox-inactive",
    sector: "exchange",
    tier: "v2",
    category: "hygiene",
    module: "exchange-deep.ts",
    permissions: ["Reports.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Inactive shared mailboxes",
    offered: true,
    graphValidationRef: "EX2",
  },
  {
    id: "hygiene.mailbox-auto-reply-stale",
    sector: "exchange",
    tier: "v2",
    category: "hygiene",
    module: "exchange-deep.ts",
    permissions: ["Mail.ReadBasic.All"],
    scoreImpact: "full",
    marketingLabel: "Stale mailbox auto-replies",
    offered: true,
    graphValidationRef: "EX1",
  },
  {
    id: "hygiene.resource-mailbox-unused",
    sector: "exchange",
    tier: "v2",
    category: "hygiene",
    module: "exchange-deep.ts",
    permissions: ["Reports.Read.All"],
    scoreImpact: "full",
    marketingLabel: "Unused resource mailboxes",
    offered: true,
    graphValidationRef: "EX2",
  },
];

export const CHECK_DEFINITION_BY_ID = new Map(
  CHECK_DEFINITIONS.map((d) => [d.id, d]),
);

export function offeredCheckDefinitions(): CheckDefinition[] {
  return CHECK_DEFINITIONS.filter((d) => d.offered);
}

export function scoredCheckCount(): number {
  return CHECK_DEFINITIONS.filter((d) => d.offered && d.scoreImpact === "full").length;
}

export const SECTOR_LABELS: Record<ScanSector, string> = {
  identity: "Identity",
  cost: "Cost",
  teams: "Teams",
  sharepoint: "SharePoint",
  exchange: "Exchange",
  devices: "Devices",
  apps: "Apps",
  copilot: "Copilot",
};

export function checkSector(checkId: string): ScanSector | undefined {
  return CHECK_DEFINITION_BY_ID.get(checkId)?.sector;
}

export function validateRegisteredCheckIds(registeredIds: string[]): string[] {
  const errors: string[] = [];
  const offered = offeredCheckDefinitions();
  const registered = new Set(registeredIds);

  for (const def of offered) {
    if (!registered.has(def.id)) {
      errors.push(`Missing implementation for offered check: ${def.id}`);
    }
  }

  for (const id of registeredIds) {
    if (!CHECK_DEFINITION_BY_ID.has(id)) {
      errors.push(`Check registered but not in registry: ${id}`);
    }
  }

  return errors;
}
